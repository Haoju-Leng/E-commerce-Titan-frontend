...
const OkButton = (props) => { //TODO: update product path
    return (<Fragment><Popup open={props.published}
                        position="top center" modal>
        <div style={{borderStyle: "solid", borderColor: "darkgray", backgroundColor: 'white', width: '500px'}}>
        <PopupHeader className="header">{`Successfully Posted!`}</PopupHeader>
            <div className="actions"><PopupButton  className="btn btn-lg btn-info" onClick={() => {props.navigate(`/item/${props.itemID}`)}}>view item</PopupButton>
                <PopupButton  className="btn btn-lg btn-info" onClick={() => {props.navigate(0)}}>post another item</PopupButton></div></div>
    </Popup>
    </Fragment>);
}
...
export const Publish = ( { user } ) => {
    ...
    return(
        <Fragment>
        {published && <OkButton published={published} navigate={navigate} itemID={itemID}/>}
                    ...
                    <form>
                        <div className="col-sm-12">
                            <div className="row">
                                <div className="col-sm-6 form-group">
                                    ...
                                    <input type="text"
                                                ...
                                                id="productName" onChange={onChange}></input>
                            ...
                            <div className="form-group">
                                ...
                                <textarea type="text"
                                               placeholder="Enter Item Description" className="form-control"
                                               id="productDescription" onChange={onChange}></textarea>
                            </div>
                            <div className="form-group">
                                <label htmlFor="productCategory">Item Category</label> TODO: multiselect or single select
                                <div><label style={{fontSize: '5px'}}><RadioInput onChange={onChange} value={"books"} checked={true}/>Books</label></div>
                                <div><label style={{fontSize: '5px'}}><RadioInput onChange={onChange} value={"electronics"} checked={false}/>Electronics</label></div>
                                <div><label style={{fontSize: '5px'}}><RadioInput onChange={onChange} value={"other"} checked={false}/>Other</label></div>
                            </div>
                            <div className="row">
                                <div className="col-sm-4 form-group">
                                    <label htmlFor="productManufacturer">Item Manufacturer</label>
                                    <input type="text"
                                                placeholder="Enter Item Manufacturer" className="form-control"
                                                id="productManufacturer" onChange={onChange}></input>

                                </div>
                                <div className="col-sm-4 form-group">
                                    <label htmlFor="productPrice">Item Price</label>
                                    <input type="text"
                                                placeholder="Enter Item Price" className="form-control"
                                                id="productPrice" onChange={onChange}></input>
                                    {priceError !== "" && <ErrorMessage msg={priceError}/>}
                                </div>
                                <div className="col-sm-4 form-group">
                                    <label htmlFor="unitStock">Number of Items</label>
                                    <input type="text"
                                                placeholder="Number of Items" className="form-control"
                                                id="unitStock" onChange={onChange}></input>
                                    {unitError !== "" && <ErrorMessage msg={unitError}/>}
                                </div>
                            </div>
                            <div className="row">
                                <label htmlFor="images">Upload images: <input style={{marginTop: '5px',fontSize: '10px'}} type="file" id="images" name="images"
                                                                              onChange={setImageState} multiple/></label>
                                {submitError !== "" && <ErrorMessage msg={submitError} />}
                            </div>
                            <div className="row justify-content-center form-actions">
                                <button className="col-1 btn btn-lg btn-info" onClick={onSubmit}>Post</button>
                            </div>
                        </div>
                    </form>
                </div>
